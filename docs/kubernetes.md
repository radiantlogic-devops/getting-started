...
### Helm
Helm can be used to streamline installing and managing Kubernetes applications. Radiant Logic offers Helm Charts to define and install RadiantOne.

#### Install Helm and Add the Radiant Logic Repo
1. Install [Helm](https://helm.sh/). Helm v3+ is required. Refer to Helm's [documentation](https://helm.sh/docs/) for assistance.

2. Add the Radiant Logic Helm Repository.
...
If you have already added this repo, run the following to retrieve the latest versions of the packages:
```
helm repo update
``` 

Run the following to see the charts:
```
...
#### Install RadiantOne
Install the ZooKeeper ensemble first, then the RadiantOne nodes.
...
* *Install ZooKeeper with custom values:*
```
helm install --namespace=<name space> <release name> radiantone/zookeeper \
--set replicaCount="5"
```
...
* *Install RadiantOne nodes with default values:*
```
helm install --namespace=<name space> <release name> radiantone/fid
```

* *Install RadiantOne nodes with custom values:*
```
helm install --namespace=<name space> <release name> radiantone/fid \
